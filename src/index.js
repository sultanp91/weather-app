...
async function getWeatherData(city) {
  try {
    const request = await fetch(
      `https://api.openweathermap.org/data/2.5/weather?q=${city}&units=metric&appid=d814b06e7180432ffac829ff0f153fb9`,
      { mode: 'cors' },
    );

    const weatherDataRequest = await request.json();

    weatherData = await dataFactory(
      weatherDataRequest.main.temp,
      weatherDataRequest.main.feels_like,
      weatherDataRequest.weather[0].main,
      weatherDataRequest.weather[0].icon,
      weatherDataRequest.name,
    );
  } catch (error) {
    console.log(error);
  }
...
locationsubmit.addEventListener('click', async (e) => {
  ...
  const cityInput = locationinput.value;
  ...
  async function success(result) {
    ...
    try {
      const locationRequest = await fetch(
        `https://eu1.locationiq.com/v1/reverse.php?key=pk.74f454060c7a9f8b245dd7bf24c08d88&lat=${latitude}&lon=${longitude}&format=json`,
      );
